...
func (tx *Tx) init() {
	tx.AcademicYear = NewAcademicYearClient(tx.config)
	...
	tx.ClubBranch = NewClubBranchClient(tx.config)
	tx.ClubType = NewClubTypeClient(tx.config)
	tx.ClubappStatus = NewClubappStatusClient(tx.config)
...
// applies a query, for example: AcademicYear.QueryXXX(), the query will be executed
