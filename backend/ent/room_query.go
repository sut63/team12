...
func (rq *RoomQuery) QueryRoomuses() *RoomuseQuery {
...
func (rq *RoomQuery) WithRoomuses(opts ...func(*RoomuseQuery)) *RoomQuery {
	query := &RoomuseQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomuses = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withRoomuses; query != nil {
		...
		query.Where(predicate.Roomuse(func(s *sql.Selector) {
			s.Where(sql.InValues(room.RoomusesColumn, fks...))
		}))
