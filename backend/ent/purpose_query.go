...
// QueryRoomuses chains the current query on the roomuses edge.
func (pq *PurposeQuery) QueryRoomuses() *RoomuseQuery {
	query := &RoomuseQuery{config: pq.config}
...
//  WithRoomuses tells the query-builder to eager-loads the nodes that are connected to
// the "roomuses" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PurposeQuery) WithRoomuses(opts ...func(*RoomuseQuery)) *PurposeQuery {
	query := &RoomuseQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withRoomuses = query
	return pq
}
...
func (pq *PurposeQuery) sqlAll(ctx context.Context) ([]*Purpose, error) {
	...
	if query := pq.withRoomuses; query != nil {
		...
		query.Where(predicate.Roomuse(func(s *sql.Selector) {
			s.Where(sql.InValues(purpose.RoomusesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Roomuses = append(node.Edges.Roomuses, n)
		}
	}
